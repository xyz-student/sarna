...
<?php 
$get_id = $_GET['id'];
    $select1 = "SELECT * FROM products WHERE id='$get_id'";
...
$sql="UPDATE `products` SET `title`='$title',`img1`='$final_img1',`des`='$desc',`cat_id`='$cat_name' WHERE id='$get_id'";
	$query=mysqli_query($conn,$sql) or die(mysqli_error($conn));
	if($query)
	{
		unlink('../assets/img/'.$old_img1);
		echo("<script>alert('Update succesfully ');window.location='product_table.php';</script>");
	}
}
else if($image=="") {
	$sql="UPDATE `products` SET `title`='$title',`img1`='$old_img1',`des`='$desc',`cat_id`='$cat_name' WHERE id='$get_id'";
	$query=mysqli_query($conn,$sql) or die(mysqli_error($conn));
	if($query)
	{
		echo("<script>alert('Update succesfully ');window.location='product_table.php';</script>");
