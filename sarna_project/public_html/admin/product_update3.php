...
<?php 
$get_id = $_GET['id'];
    $select1 = "SELECT * FROM `product3` WHERE id='$get_id'";
    ...
    <div class="container-fluid">
			...
			<div class="card">
				...
				<form method="POST" enctype="multipart/form-data">
					...
					<div class="col-sm-10">
					 	<input type="text" name="title" class="form-control" value="<?=$row1['title3'];?>">
					</div>
				  </div>
				...
				<div class="form-group row">
                  ...
                  <div class="col-sm-4">
                    <input type="file" class="form-control" name="image1" value="<?=$row1['img3'];?>">
                  </div>
                  <div class="col-sm-4">
                  	<img src="../assets/img/<?=$row1['img3'];?>" style="width:60px;">
                  </div>
                </div>
...
$sql="UPDATE `product3` SET `title3`='$title',`img3`='$final_img1',`des3`='$desc',`cat_id3`='$cat_name' WHERE id='$get_id'";
	$query=mysqli_query($conn,$sql) or die(mysqli_error($conn));
	if($query)
	{
		unlink('../assets/img/'.$old_img1);
		echo("<script>alert('Update succesfully ');window.location='product_table3.php';</script>");
	}
}
else if($image=="") {
	$sql="UPDATE `product3` SET `title3`='$title',`img3`='$old_img1',`des3`='$desc',`cat_id3`='$cat_name' WHERE id='$get_id'";
	$query=mysqli_query($conn,$sql) or die(mysqli_error($conn));
	if($query)
	{
		echo("<script>alert('Update succesfully ');window.location='product_table3.php';</script>");
